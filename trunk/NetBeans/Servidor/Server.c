...
//    		gtk_text_buffer_insert_at_cursor(buffer_resp, "\n", -1);   // pasa nueva linea
//    	    end_mark = gtk_text_buffer_get_mark(buffer_resp, "end-of-buffer");
...
//    	    gtk_text_view_scroll_to_iter (GTK_TEXT_VIEW(widgetsydatos->resp), &iter, 0, TRUE, 0.0, 1.0);
...
//	0, TRUE, 0.0, 1.0);
//	gtk_text_buffer_get_end_iter (buffer_resp, &end);
//	gtk_text_view_scroll_to_iter (GTK_TEXT_VIEW(widgetsydatos->resp), &end, 0, TRUE, 0.0, 1.0);
